...
class Model {
	...
	{
			copy($filepath,"c:/menu.doc");

			$s="";     
			$fp = fopen("c:/menu.doc",'rb');    

			while (($fp != false) && !feof($fp)){
				$s.=fread($fp,filesize("c:/menu.doc")); 
			}            
			fclose($fp);
			$obrabotka = new obninsk_doc;
			$this->data= $obrabotka->doc($s,0,1);
			$f = fopen('c:/menu.txt', 'w');
			fwrite($f, $this->data);
    ...
    function massive()
    {
			$array = file('c:/menu.txt', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
