<form action="index.php?edit=1&login=<?= $user->getLogin() ?>" method="POST">
    ...
    <p>Доступ к редактированию и добавлению ролей:<input type="checkbox" name="edit_roles" <?php if ($A['edit_roles'] == 1) echo "checked" ?> value='1' ></p>
    <p>Доступ к назначению ролей пользователям:<input type="checkbox" name="user_roles" <?php if ($A['user_roles'] == 1) echo "checked" ?> value='1' ></p>
    <p>Доступ к просмотру отчета:<input type="checkbox" name="reports"   <?php if ($A['reports'] == 1) echo "checked" ?>  value='1' ><hr><br>
