    ...
    foreach ($users as $user) {
        ?>
                <form action="index.php?user_roles=1" method="POST">
                    <tr>
                        <td><?= $user->getLogin() ?></td>
                        <td><a href="index.php?user_edit=1&login=<?= $user->getLogin() ?>"><?= $user->getName() ?></td>
                        <td><?= $user->getSurname() ?></td>
                        <td><?= $user->getEmail() ?></td> 
                        <td><select size="1" name="role">
        <?php foreach ($roles as $role) {
            if ($role[role_name] == $user->getRole()) {
                echo "<option selected value=" . $user->getId() . "|" . $role[id] . ">$role[role_name]</option>";
            } else {
                echo "<option value=" . $user->getId() . "|" . $role[id] . ">$role[role_name]</option>";
            }
        }
        ?>
                            </select></td>
                        <td><input type='submit' name="input_role" value="Переназначить роль"></td>
                        <td><a href="index.php?user_del=1&login=<?= $user->getLogin() ?>">Удалить</td>
                    </tr>
                </form> 
        <?php
    }
